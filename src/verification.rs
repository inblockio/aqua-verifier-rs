...
use aqua_verifier_rs_types::{
    crypt,
    models::{
        hash::Hash, public_key::PublicKey, revision::Revision, signature::Signature,
        timestamp::Timestamp, tx_hash::TxHash,
    },
};
use sha3::Digest;
use std::collections::{BTreeMap, HashSet};

use crate::revision_integrity::RevisionIntegrity;
...
pub fn only_verification_hash_integrity(
    rev: &Revision,
...
pub fn only_signature_hash_integrity(
    rev: &Revision,
    prev: Option<&Revision>,
) -> flagset::FlagSet<RevisionIntegrity> {
    let mut integrity = flagset::FlagSet::default();
...
///
/// prerequisites: rev [trusted verification_hash]
pub fn only_witness_hash_integrity(
    rev: &Revision,
    prev: Option<&Revision>,
) -> flagset::FlagSet<RevisionIntegrity> {
    let mut integrity = flagset::FlagSet::default();
